...
public class JumperGameManager : MonoBehaviour 
{
    ...
    public int starsToCapture;

    [Space(20)]
    public SpriteRenderer backgroundImage;
    public SpriteRenderer graphics; //Cabeza jugador
    public static JumperGameManager instance;
    ...
    private void Awake() {
        ...
        if (JumperGameManager.instance == null)
            JumperGameManager.instance = this;
        else if (JumperGameManager.instance != null)
            Destroy(gameObject);
    }
	...
	void Update ()
	{
        ...
        if (!Game.isGameStarted)
            return;
        //else:
        graphics.sprite = player.heads.shop.characters[GameManager.instance.playerSelectedIndex].characterSprite;
        ...
        if ((power > 0.99F && !chill && enableAutoJump) || Input.GetMouseButtonDown(0))
            player.Jump();
